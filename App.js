...
// Note: Analytics might require specific environment setup in React Native/Expo
// If it causes errors in development, you can comment the next line out.
const analytics = getAnalytics(app); 
const auth = getAuth(app);
// --- FIREBASE CONFIGURATION END ---
// Voeg toe aan je firebase imports
...
export const PageWithBack = ({ currentPage, pageName, setCurrentPage, title, bio, type, archived, mlUser }) => {
  ...
  const sendMessage = async () => {
    ...
    await set(msgRef, [...current, newMsg]);
  ...
  */}
  ...
  // Run algorithm whenever userId, tasks, or goals change'
    ...
    const setupAndRun = async () => {
        ...
        await set(ref(db, 'users/' + userData.id), userData);
        console.log("✅ User updated in Firebase");

        // Get all users + groups from Firebase
    ...
    // Auto-refresh users + groups every 10s
    useEffect(() => {
    const interval = setInterval(async () => {
        ...
        setItems(prev => {
            ...
            if (item.type === "person") {
                const u = usersData.find(u => u.id === item.id);
                if (u) return { ...item, name: u.name, bio: u.bio };
            }
            if (item.type === "group") {
                const g = groupsData.find(g => g.id === item.id);
                if (g) return { ...item, name: g.name, bio: g.bio };
            }
            return item;
            });
